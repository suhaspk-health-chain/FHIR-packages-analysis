...
# ---- Data Dependency: load all_resources ----
# Generated by 08_plot_version_resource_comparison.R (run that first)
all_resources_rds <- file.path(PROC_DIR, "all_resources.rds")
if (file.exists(all_resources_rds)) {
  all_resources <- readRDS(all_resources_rds)
} else if (file.exists(file.path(PROC_DIR, "resource_presence_matrix.csv"))) {
  # Reconstruct from presence matrix if RDS not available
  message("all_resources.rds not found — reconstructing from resource_presence_matrix.csv")
  pmat <- read.csv(file.path(PROC_DIR, "resource_presence_matrix.csv"), check.names = FALSE)
  all_resources <- tidyr::pivot_longer(pmat, -resource, names_to = "version", values_to = "present") |>
    dplyr::filter(present == 1L) |>
    dplyr::select(version, resource)
} else {
  stop("No all_resources.rds or resource_presence_matrix.csv found. Run 08_plot_version_resource_comparison.R first.")
}
